...
# Truncate long quotes at sentence boundary near this length.
TRUNCATE_SOFT_LIMIT = 200
TRUNCATE_BUFFER = 100
MAX_CATEGORY_TAGS = 3
...
def truncate_quote(
    text: str,
    soft_limit: int = TRUNCATE_SOFT_LIMIT,
    buffer: int = TRUNCATE_BUFFER,
) -> str:
    """
...
def display_results(results: list[dict]) -> None:
    ...
    for i, r in enumerate(results, 1):
        ...
        if category:
            tags = [t.strip() for t in category.split(",")][:MAX_CATEGORY_TAGS]
